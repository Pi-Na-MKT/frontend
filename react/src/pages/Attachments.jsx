  ...
  return (
    <div className="px-5 pb-5 pt-2">
      ...
      <div
        onDragOver={(e) => {
          e.preventDefault()
          setDragOverId(empresa.id)
        }}
        onDragLeave={() => setDragOverId(null)}
        onDrop={(e) => {
          e.preventDefault()
          setDragOverId(null)
          onUpload(empresa.id, e.dataTransfer.files)
        }}
        onClick={() => {
          fileInputRef.current.dataset.empresa = empresa.id
      ...
      >
        {isActive ? (
          ...
          <>
            <div
              className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors ${isDrag ? 'bg-primary text-white' : 'bg-gray-100 text-gray-400'}`}
            >
              <svg
                className="w-4 h-4"
                fill="none"
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-600">
                Arraste ou{' '}
                <span className="text-primary">clique para anexar</span>
              </p>
          ...
          {lista.map((a, i) => {
            ...
            return (
              ...
              >
                {/* Ícone tipo */}
                <div
                  className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 bg-white border ${cfg.border} ${cfg.text}`}
                >
                  {cfg.icon}
                </div>
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button
                    onClick={() => onDelete(empresa.id, a.id)}
...
export default function Anexos() {
  ...
  const handleDelete = async (empresaId, anexoId) => {
    try {
      ...
      setAnexos((prev) => ({
        ...prev,
        [empresaId]: prev[empresaId].filter((a) => a.id !== anexoId),
      }))
    } catch (err) {
  ...
  const totalAnexos = Object.values(anexos).reduce(
    (s, arr) => s + arr.length,
    0
  )
  const empresasComAnexos = empresas.filter(
    (e) => (anexos[e.id] || []).length > 0
  ).length
    ...
    <div className="p-6 lg:p-8 max-w-5xl mx-auto animate-fade-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
        <div>
          ...
          <p className="page-sub">
            Arquivos e documentos organizados por empresa
          </p>
        </div>
        <div className="flex gap-3">
          <div className="bg-violet-50 border border-violet-100 rounded-xl px-4 py-2.5 text-center">
            ...
            <p className="text-[10px] font-semibold text-violet-400 mt-0.5 uppercase tracking-wide">
              Arquivos
            </p>
          </div>
          <div className="bg-emerald-50 border border-emerald-100 rounded-xl px-4 py-2.5 text-center">
            ...
            <p className="text-[10px] font-semibold text-emerald-400 mt-0.5 uppercase tracking-wide">
              Empresas
            </p>
          </div>
        </div>
          ...
          return (
            ...
            >
              ...
              >
                {/* Avatar empresa */}
                <div
                  className={`w-10 h-10 rounded-xl ${emp.cor} flex items-center justify-center text-white text-sm font-bold flex-shrink-0 shadow-sm`}
                >
                  {emp.inicial}
                </div>
                ...
                {count > 0 && (
                  <span
                    className={`badge mr-2 ${isOpen ? 'bg-primary/10 text-primary ring-1 ring-primary/20' : 'bg-gray-100 text-gray-500 ring-1 ring-gray-200/60'}`}
                  >
                    {count}
                  </span>
                )}
              ...
              {isOpen && (
                <div className="border-t border-gray-100">
                  <EmpresaPanel
                    ...
                    onDelete={(eId, aId) =>
                      setConfirmDel({ empresaId: eId, anexoId: aId })
                    }
      ...
      {confirmDel && (
        <ConfirmDialog
          ...
          onConfirm={() =>
            handleDelete(confirmDel.empresaId, confirmDel.anexoId)
          }
