...
function AvatarUpload({ preview, onFile }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3 mb-2">
      <div className="relative w-20 h-20 rounded-full cursor-pointer group"
...
export default function Cadastro({ onGoToLogin, isInternalAccess = false }) {
  ...
  const toggleCanal = (c) =>
    setForm(p => ({ ...p, canais: p.canais.includes(c) ? p.canais.filter(x => x !== c) : [...p.canais, c] }))
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        responsibility: form.canais.length > 0 ? form.canais.join(', ') : null,
        bio:            form.bio        || null,
  ...
  const resetForm = () => {
    setSucesso(false); setStep(0); setAvatarPreview(null)
    setForm({ nome: '', email: '', telefone: '', senha: '', confirmarSenha: '', cargo: '', senioridade: '', canais: [], bio: '', linkedin: '' })
  }
      ...
      <div style={{ fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", background: '#0A0A0F' }}
        ...
        <div className="text-center max-w-sm">
          <div className="relative mx-auto w-20 h-20 mb-6">
            <div className="absolute inset-0 rounded-full animate-ping opacity-20" style={{ background: 'linear-gradient(135deg, #5B4FE8, #a78bfa)' }} />
            {usuarioCriado?.avatar ? (
              ...
              <div className="relative w-20 h-20 rounded-full flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #5B4FE8, #a78bfa)' }}>
          ...
          <div className="rounded-2xl p-4 mb-6 text-left" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
            {[['E-mail', usuarioCriado?.email], ['Cargo', usuarioCriado?.cargo]].map(([k, v]) => v && (
              <div key={k} className="flex justify-between py-1.5 border-b last:border-0" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
          ...
          <div className="flex flex-col gap-2.5">
            <button onClick={resetForm} className="w-full py-3 rounded-xl text-sm font-semibold text-white transition-all"
              style={{ background: 'linear-gradient(135deg, #5B4FE8, #7c6ff7)', boxShadow: '0 0 25px rgba(91,79,232,0.3)' }}>
              Cadastrar outro
            </button>
            <button onClick={onGoToLogin} className="w-full py-3 rounded-xl text-sm font-semibold transition-all"
  ...
  return (
    <div style={{ fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", background: '#0A0A0F' }}
      ...
      <div className="hidden lg:flex lg:w-[42%] relative overflow-hidden flex-col">
        <div className="absolute inset-0" style={{ background: 'linear-gradient(160deg, #0f0c29 0%, #302b63 60%, #1a1040 100%)' }} />
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute w-[500px] h-[500px] rounded-full opacity-20" style={{ background: 'radial-gradient(circle, #5B4FE8 0%, transparent 70%)', top: '-80px', left: '-80px' }} />
          <div className="absolute w-[400px] h-[400px] rounded-full opacity-10" style={{ background: 'radial-gradient(circle, #a78bfa 0%, transparent 70%)', bottom: '0', right: '-60px' }} />
        </div>
        <div className="relative z-10 flex flex-col h-full p-12 justify-between">
          <div className={`transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #5B4FE8, #a78bfa)' }}>
                <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex-1 flex flex-col justify-center">
            <div className={`transition-all duration-500 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
              <h1 className="text-4xl font-bold text-white leading-tight mb-4">
                Crie seu<br />
                <span style={{ background: 'linear-gradient(135deg,#a78bfa,#38bdf8)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>perfil</span>
              </h1>
            ...
            <div className="mt-12 flex flex-col gap-3">
              {STEPS.map((s, i) => (
                <div key={s} className="flex items-center gap-3 transition-all duration-300" style={{ opacity: i <= step ? 1 : 0.3 }}>
                  <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 transition-all duration-300"
                    style={{ background: i < step ? '#10b981' : i === step ? 'linear-gradient(135deg,#5B4FE8,#a78bfa)' : 'rgba(255,255,255,0.08)', color: i <= step ? '#fff' : '#6b7280' }}>
                    {i < step ? <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg> : i + 1}
                  </div>
                  <div>
                    <p className="text-sm font-semibold" style={{ color: i === step ? '#fff' : '#6b7280' }}>{s}</p>
                    <p className="text-xs" style={{ color: '#374151' }}>{['Dados pessoais e foto', 'Cargo e senioridade', 'Habilidades e bio'][i]}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 relative overflow-hidden">
        <div className="absolute pointer-events-none" style={{ width: '500px', height: '500px', background: 'radial-gradient(circle, rgba(91,79,232,0.06) 0%, transparent 70%)', top: '50%', left: '50%', transform: 'translate(-50%,-50%)' }} />
        <div className={`relative z-10 w-full max-w-[420px] transition-all duration-700 delay-100 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          ...
          <div className="lg:hidden flex items-center justify-between mb-8">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'linear-gradient(135deg,#5B4FE8,#a78bfa)' }}>
          ...
          <div style={{ transition: 'opacity 0.22s, transform 0.22s', opacity: anim ? 0 : 1, transform: anim ? `translateX(${dir * 30}px)` : 'translateX(0)' }}>
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <Field label="Confirmar" error={erros.confirmarSenha}>
                    <div className="relative">
                      <input type={showConfirmar ? 'text' : 'password'} value={form.confirmarSenha}
                  ...
                  <div className="grid grid-cols-3 gap-1.5">
                    {SENIORIDADE.map(s => (
                      <button key={s.value} type="button" onClick={() => set('senioridade', s.value)}
                        ...
                        style={{ background: form.senioridade === s.value ? 'rgba(91,79,232,0.15)' : 'rgba(255,255,255,0.04)', border: `1.5px solid ${form.senioridade === s.value ? '#5B4FE8' : 'rgba(255,255,255,0.08)'}` }}>
            ...
            {step === 2 && (
              <div className="flex flex-col gap-4">
                    ...
                    {HABILIDADES.map(c => {
                      ...
                      return (
                        <button key={c} type="button" onClick={() => toggleCanal(c)}
                          ...
                          style={{ background: sel ? 'rgba(91,79,232,0.2)' : 'rgba(255,255,255,0.04)', border: `1px solid ${sel ? '#5B4FE8' : 'rgba(255,255,255,0.08)'}`, color: sel ? '#a78bfa' : '#6b7280' }}>
                          {c}
                        </button>
                      )
                    })}
                ...
                <Field label="LinkedIn" hint="opcional">
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-medium" style={{ color: '#4b5563' }}>linkedin.com/in/</span>
          ...
          <div className="flex gap-3 mt-6">
            ...
            {step < 2 ? (
              ...
              <button type="button" onClick={handleSubmit} disabled={loading}
                ...
                {loading ? (
                  <><svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>Cadastrando...</>
                ) : (
                  <>Criar conta<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg></>
                )}
              </button>
            )}
          </div>
          ...
          {step === 0 && (
            <p className="text-center text-sm mt-5" style={{ color: '#374151' }}>
              ...
              <button type="button" onClick={onGoToLogin}
                className="font-semibold transition-colors hover:text-white" style={{ color: '#7c6ff7' }}>
