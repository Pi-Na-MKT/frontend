...
export default function Tarefas({ empresa, onBack, onDashboard }) {
  ...
  const totalCards = Object.values(boardHook.cardsByColumn).reduce(
    (s, arr) => s + arr.length,
    0
  )
  const doneColumnId = boardHook.columns.find(
    (c) =>
      c.label.toLowerCase().includes('conclu') ||
      c.label.toLowerCase().includes('finaliz')
  )?.id
  const doneCount = doneColumnId
    ? (boardHook.cardsByColumn[doneColumnId] || []).length
    : 0
  const donePercent =
  ...
  return (
    <div className="h-full flex flex-col animate-fade-up">
      <div className="flex-shrink-0 bg-white border-b border-gray-100 px-6 py-4">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            {empresa && (
              <div
                className={`w-9 h-9 rounded-xl ${empresa.cor} flex items-center justify-center text-white text-sm font-bold shadow-sm`}
              >
                {empresa.inicial}
              </div>
            )}
            ...
            {canManage ? (
              <>
                <p className="text-gray-400 text-xs mb-5">
                  Crie o board desta empresa para começar a organizar as
                  tarefas.
                </p>
                <button
                  onClick={boardHook.handleCreateBoard}
                  disabled={boardHook.creatingBoard}
                  className="btn-primary disabled:opacity-50"
                >
                  {boardHook.creatingBoard ? (
              ...
              <p className="text-gray-400 text-xs">
                Aguarde um administrador criar o board desta empresa.
              </p>
            )}
          </div>
        </div>
                ...
                {boardHook.addingColumn ? (
                  <div className="bg-gray-100/70 rounded-2xl p-4">
                    <input
                      autoFocus
                      type="text"
                      value={boardHook.newColumnName}
                      onChange={(e) =>
                        boardHook.setNewColumnName(e.target.value)
                      }
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') boardHook.handleAddColumn()
                        if (e.key === 'Escape') {
                          boardHook.setAddingColumnState(false)
                        }
                      }}
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={boardHook.handleAddColumn}
                        disabled={
                          boardHook.savingColumn ||
                          !boardHook.newColumnName.trim()
                        }
                  ...
                  <button
                    onClick={() => boardHook.setAddingColumnState(true)}
      ...
      {boardHook.cardModal && (
        <CardModal
          ...
          registeredUsers={registeredUsers.filter((u) =>
            boardHook.boardMemberIds.has(Number(u.id))
          )}
        ...
        <ConfirmDialog
          ...
          description={
            <p className="text-sm text-gray-500">
              "{boardHook.deleteCardTarget.card.titulo}" será removido
              permanentemente.
            </p>
          }
      ...
      {boardHook.deleteColumnTarget &&
        (() => {
          const col = boardHook.columns.find(
            (c) => c.id === boardHook.deleteColumnTarget
          )
          const qty = (
            boardHook.cardsByColumn[boardHook.deleteColumnTarget] || []
          ).length
          return (
            <ConfirmDialog
                ...
                <div className="flex flex-col gap-1">
                  {qty > 0 && (
                    <p className="text-sm text-red-500 font-medium">
                      {qty} card{qty !== 1 ? 's' : ''} serão perdidos.
                    </p>
                  )}
