...
export default function Dashboard({ empresaInicial = null, onBack = null }) {
  ...
  const kpis = useMemo(() => {
    const now = new Date()
    const allCards = dadosFiltrados.flatMap((d) =>
      d.colunas.flatMap((c) => c.cards)
    )
    const total = allCards.length
    const concluidas = allCards.filter((c) => c.completed).length
    const atrasadas = allCards.filter(
  ...
  const chartData = useMemo(() => {
    return dadosFiltrados.map((d) => {
      ...
      return {
        label:
          d.empresa.nome.length > 12
            ? d.empresa.nome.slice(0, 10) + '…'
            : d.empresa.nome,
        concluidas,
    ...
    <div className="h-full flex flex-col overflow-hidden bg-[#F4F5F7]">
      ...
      {!loading && (
        <div className="flex-1 overflow-hidden p-4 flex flex-col gap-4 min-h-0">
          ...
          {/* Gráfico */}
          <div
            className="flex-shrink-0 bg-white rounded-xl border border-gray-100 p-4"
            style={{ height: '210px' }}
          >
            ...
            {chartData.length > 0 &&
            chartData.some((d) => d.concluidas + d.pendentes > 0) ? (
              <ResponsiveContainer width="100%" height={170}>
                <BarChart
                  data={chartData}
                  margin={{ top: 4, right: 10, left: -25, bottom: 0 }}
                >
                  ...
                  <Legend
                    ...
                    formatter={(v) =>
                      v === 'concluidas' ? 'Concluídas' : 'Pendentes'
                    }
                  />
                  <Bar
                    dataKey="concluidas"
            ...
            <div className="px-5 py-3 border-b border-gray-100 flex-shrink-0 flex items-center justify-between">
              ...
              {membrosData.length > 0 && (
                <span className="text-[11px] text-gray-400">
                  {membrosData.length} membro
                  {membrosData.length !== 1 ? 's' : ''}
                </span>
              )}
            </div>
                ...
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-white">
                    <tr className="border-b border-gray-100">
                      {[
                        ...
                        <th
                          key={h}
                          className="text-left py-2.5 px-4 text-[10px] font-semibold text-gray-400 uppercase tracking-wider"
                        >
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {membrosData.map((m, i) => {
                      ...
                      const taxa =
                        total > 0 ? Math.round((m.concluidas / total) * 100) : 0
                      return (
                        <tr
                          key={i}
                        ...
                        >
                          ...
                          <td className="py-3 px-4 text-emerald-600 font-semibold text-xs">
                            {m.concluidas}
                          </td>
