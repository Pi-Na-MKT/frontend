  ...
  return (
    <div
      className={`flex flex-col bg-gray-100/70 rounded-2xl flex-shrink-0 w-80 transition-all ${
        dragDropHook.dragColOverId === col.id && dragDropHook.draggingColId !== col.id ? 'ring-2 ring-primary/40' : ''
      }`}
      ...
      onDragLeave={(e) => {
        if (dragDropHook.dragColRef?.current && !e.currentTarget.contains(e.relatedTarget))
    ...
    >
      ...
      <div className="flex items-center gap-2 px-4 pt-4 pb-3 flex-shrink-0">
        {canManage && (
          <div
            draggable
            onDragStart={(e) => { e.stopPropagation(); dragDropHook.handleColDragStart(col.id) }}
            onDragEnd={dragDropHook.handleColDragEnd}
            className={`cursor-grab active:cursor-grabbing flex-shrink-0 transition-colors ${
              dragDropHook.draggingColId === col.id ? 'text-primary' : 'text-gray-300 hover:text-gray-500'
            }`}
        ...
        {isEditing ? (
          <input
            autoFocus type="text" value={boardHook.editingName}
        ...
        {!isEditing && canManage && (
          <div className="flex items-center gap-0.5">
            <button
              onClick={() => { boardHook.setEditingColumn(col.id, col.label) }}
      ...
      >
        ...
        {cards.map(tarefa => (
          <div
            key={tarefa.id}
            draggable
            onDragStart={() => dragDropHook.handleCardDragStart(tarefa.id, col.id)}
          ...
          >
            ...
            {dragDropHook.dragOverCardId === tarefa.id && dragDropHook.draggingCardId !== tarefa.id && (
              <div className="h-0.5 bg-primary rounded-full mb-1.5 mx-1"/>
            )}
            
            <TaskCard
              tarefa={tarefa}
              onEdit={(t) => boardHook.setCardModal({ columnId: col.id, card: t, position: t.position })}
              onDelete={(t) => boardHook.setDeleteCardTarget({ card: t, columnId: col.id })}
        ...
        <button
          onClick={() => boardHook.setCardModal({ columnId: col.id, card: null, position: cards.length })}
